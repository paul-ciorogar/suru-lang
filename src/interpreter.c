...
static int execute_function_call(Interpreter *interp, int func_node_idx, Array *arg_values,
...
static Scope *create_scope(Interpreter *interp, Scope *parent, int scope_id) {
    ...
    if (!scope->variables) return NULL;
...
static void store_variable(Scope *scope, String *name, ValueType type,
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (var->name == name) {
            var->type = type;
            if (type == VALUE_STRING) var->string_value = str_val;
            else if (type == VALUE_BOOLEAN) var->bool_value = bool_val;
            else if (type == VALUE_FUNCTION) var->func_value = func_val;
            return;
        }
    }
    ...
    new_var.name = name;
    new_var.type = type;
    if (type == VALUE_STRING) new_var.string_value = str_val;
    else if (type == VALUE_BOOLEAN) new_var.bool_value = bool_val;
    else if (type == VALUE_FUNCTION) new_var.func_value = func_val;
...
Interpreter *create_interpreter(Arena *arena, AST *ast) {
    ...
    if (!interp->global_scope) return NULL;
...
static int execute_program(Interpreter *interp) {
    ...
    int result = execute_function_call(interp, main_sym->ast_node_idx, empty_args,
...
static int execute_function_call(Interpreter *interp, int func_node_idx, Array *arg_values,
    ...
    ASTNode *func_node = get_ast_node(interp->ast, func_node_idx);
    if (!func_node || func_node->type != AST_FUNCTION_DECL) {
        fprintf(stderr, "Error: Invalid function node\n");
        return 1;
    }

    // Get function name (for scope lookup)
    ...
    // Look up function in symbol table to get its scope_id
    Symbol *func_sym = lookup_function(interp->ast->symbols, name_node->token.text, 0);
    if (!func_sym) {
        fprintf(stderr, "Error: Function not in symbol table\n");
        return 1;
    }
    ...
    case AST_MATCH_STMT: {
        ...
        while (arm_idx != -1) {
            ...
            if (matches) {
                ...
                return execute_statement(interp, stmt_idx, out_type, out_string,
                                       out_bool, out_func, has_return);
            }
...
static int evaluate_expression(Interpreter *interp, int node_idx,
    ...
    case AST_IDENTIFIER: {
        ...
        *out_type = var->type;
        if (var->type == VALUE_STRING) *out_string = var->string_value;
        else if (var->type == VALUE_BOOLEAN) *out_bool = var->bool_value;
        else if (var->type == VALUE_FUNCTION) *out_func = var->func_value;
        return 0;
    }
    ...
    case AST_CALL_EXPR: {
        ...
        if (func_name->token.text && func_name->token.text->length == 5 &&
            ...
            if (arg_type == VALUE_STRING) print_string(arg_str);
            else if (arg_type == VALUE_BOOLEAN) print_boolean(arg_bool);
            else {
        ...
        while (arg_idx != -1) {
            ...
            if (arg_type == VALUE_STRING) arg_var.string_value = arg_str;
            else if (arg_type == VALUE_BOOLEAN) arg_var.bool_value = arg_bool;
            else if (arg_type == VALUE_FUNCTION) arg_var.func_value = arg_func;
        ...
        // Call the function
        int has_return = 0;
        int result = execute_function_call(interp, func_sym->ast_node_idx, arg_values,
                                          out_type, out_string, out_bool, out_func, &has_return);
        array_free(arg_values);
        return result;
    }
