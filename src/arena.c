...
#define ARENA_PAGE_SIZE (getpagesize())
...
Arena *arena_create() {
  ...
  arena->first_chunk = chunk_create(ARENA_PAGE_SIZE);
...
void *arena_alloc(Arena *arena, size_t size) {
  ...
  if (arena->current_chunk->used + aligned_size <= arena->current_chunk->size) {
    void *ptr =
  ...
  // Current chunk doesn't have enough space, try to find an existing chunk with
  // space
