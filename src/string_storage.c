...
static bool strings_equal(const char* a, size_t len_a, const char* b, size_t len_b) {
    if (len_a != len_b) return false;  // Fast rejection by length
    return memcmp(a, b, len_a) == 0;
}

// Find existing string in linked list
static String* find_string(StringStorage* storage, const char* data, size_t length) {
    StringNode* current = storage->head;
    while (current) {
        if (strings_equal(current->string.data, current->string.length, data, length)) {
...
static String* create_string(StringStorage* storage, const char* data, size_t length) {
    // Allocate new node
...
StringStorageStats get_storage_stats(StringStorage* storage) {
    ...
    StringNode* current = storage->head;
    while (current) {
        stats.memory_used_strings += sizeof(StringNode) + current->string.length + 1;
        current = current->next;
    }
...
void debug_print_strings(StringStorage* storage) {
    ...
    int index = 0;
    while (current) {
        printf("  [%d] len=%zu: \"%s\"\n", index++, current->string.length, current->string.data);
        current = current->next;
    }
}